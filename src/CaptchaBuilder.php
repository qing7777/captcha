...
use Qing7777\Captcha\Exception\CaptchaException;
use Symfony\Component\Finder\Finder;

/**
 * Class Captcha
 * @package Qing7777\Captcha
 */
class CaptchaBuilder implements CaptchaInterface
{
    ...
    {
        return $options['rounds'] ?? 10;
    }

    protected function makeHash($value, array $options = [])
    {
        ...
        if ($hash === false) {
            throw new CaptchaException('Bcrypt hashing not supported.');
        }
    ...
    public function check($value)
    {
        ...
        $check = $this->checkHash($value, $key);
    ...
    {
        return $this->checkHash($value, $key);
    }
    ...
    public function src($config = null)
    {
        ...
        return $request->url() . '/captcha' . ($config ? '/' . $config : '/default') . '?' . $this->str->random(8);
    }
